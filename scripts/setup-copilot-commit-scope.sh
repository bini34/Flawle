...
# Configure Copilot commit message template to infer scopes from file paths
# Monorepo-aware: mobile/* and backend/*
...
# Remove deprecated rules file if present (no longer used by Copilot)
if [ -f .copilot/commit-message-rules.json ]; then
  if git ls-files --error-unmatch .copilot/commit-message-rules.json >/dev/null 2>&1; then
    git rm -f .copilot/commit-message-rules.json || true
  fi
  rm -f .copilot/commit-message-rules.json || true
  echo "Removed deprecated .copilot/commit-message-rules.json"
fi

# Ensure the template exists; create if missing
if [ ! -f .copilot/commit-message-template.md ]; then
  cat > .copilot/commit-message-template.md <<'EOF'
<!--
...
EOF
  echo "Wrote .copilot/commit-message-template.md"
else
  echo ".copilot/commit-message-template.md already exists; not overwriting"
fi

echo "Copilot commit scope template is configured. In VS Code: stage changes, focus the commit message box, press Tab to accept." 
